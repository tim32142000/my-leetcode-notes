SortedList 包含在第三方套件 sortedcontainers，可能需要安裝。  
```python
from sortedcontainers import SortedList
```
leetcode 中不用 import 直接使用。  

SortedList Class 有 bisect_left 方法，在 leetcode上快很多。
語法： `sorted_list_instance.bisect_left(value)`
